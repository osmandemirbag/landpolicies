"""
Extract individual-level roll call votes for a SINGLE YEAR from BSB OCR.

This script processes only the bills for the specified year from the
KNOWN_AG_ROLLCALLS catalogue, making it much faster than processing
all 119 bills at once. Run each year separately, then use
merge_individual_votes.py to combine them.

Usage:
    python extract_year_votes.py --year 1931
    python extract_year_votes.py --year 1925 --force

    # Extract all years one by one:
    for y in $(seq 1919 1933); do python extract_year_votes.py --year $y; done

Primary data source:
  BSB Digitale Sammlungen (CC BY-SA 4.0)
...
def get_bills_for_year(year):
    """Return list of (global_index, entry) for bills in the given year."""
    result = []
...
def extract_year(year, force=False):
    """Extract individual votes for all bills in a given year.

    Writes data/individual_by_year/ag_individual_YYYY.csv.
    Returns the list of records.
    """
    out_file = os.path.join(INDIVIDUAL_YEAR_DIR, f"ag_individual_{year}.csv")

    if os.path.exists(out_file) and not force:
        print(f"  {out_file} already exists — skipping (use --force to re-extract)")
        return []

    bills = get_bills_for_year(year)
    if not bills:
        print(f"  No bills catalogued for year {year}")
        return []

    print(f"\n{'='*60}")
    print(f"  Year {year}: {len(bills)} agricultural roll call bills")
    print(f"{'='*60}")
    ...
    n_mp = sum(1 for r in all_records if r["mp_name"])
    print(f"\n  Summary for {year}:")
    print(f"    Bills with data:    {bills_with_data}")
    print(f"    Bills metadata-only:{bills_without_data}")
    print(f"    Individual records: {n_mp}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract individual roll call votes for a single year from BSB OCR")
    parser.add_argument("--year", type=int, required=True,
                        help="Year to extract (1919-1933)")
    parser.add_argument("--force", action="store_true",
                        help="Re-extract even if output file exists")
    args = parser.parse_args()

    if args.year < 1919 or args.year > 1933:
        print(f"Error: year must be between 1919 and 1933, got {args.year}")
        sys.exit(1)

    print("=" * 60)
    print("Weimar Agricultural Roll Call Votes – Year-by-Year Extraction")
    print("=" * 60)
    print(f"  Target year: {args.year}")
    print(f"  Force re-extract: {args.force}")
    print(f"  Data source: BSB Digitale Sammlungen (CC BY-SA 4.0)")

    extract_year(args.year, force=args.force)

    print("\nDone!")
    print("After extracting all years, run: python merge_individual_votes.py")
